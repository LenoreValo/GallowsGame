# Игра "Виселица" или угадайка слов
import random

# глобальный список, содержащий слова, которые будут использоваться в игре
...
#  Функция, которая возвращает случайное слово из списка word_list в верхнем регистре
def get_word():
    return random.choice(word_list).upper()

# Функция, возвращающая текущее состояние игрока (рисует человечка на виселице)
...
# Функция заменяющая _ на отгаданную букву в слове и печтающая это самое слово с отгаданными буквами
def change_word(secret_word, letter_input, word_compl):
    for i in range(len(secret_word)):
        if letter_input == secret_word[i]:
            word_compl[i] = letter_input
    return word_compl
...
def play(word):
    ...
    print('Текущее состояние игрока:')
    print(display_hangman(tries))
    print('Задуманное слово: ', *word_completion, f' состоит из {len(word)} букв.', sep='')
    ...
    while tries > 0:
        ...
        while True:
            letter_word = input('Введите букву или слово: ')
            if letter_word.isalpha():
                letter_word = letter_word.upper().strip()
                break
            else:
                print('Некорректно введена буква/слово. Повторите еще раз.')
        if len(letter_word) == 1:
            if letter_word not in guessed_letters:
                guessed_letters.append(letter_word)
                if letter_word in word:
                    print('Вы угадали букву!')
                    word_completion = change_word(word, letter_word, word_completion)
            ...
            if letter_word not in guessed_words:
...
play(get_word())
while True: 
    agree = input('Хотите заново сыграть? (Да / Нет) ').lower()
    if agree == 'да':
        play(get_word())
    else:
